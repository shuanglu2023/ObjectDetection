...
parser.add_argument('bop_parent_path', nargs='?', default="/home/ziyu/shuang/gdrnpp_bop2022/datasets/BOP_DATASETS", help="Path to the bop datasets parent directory")
parser.add_argument('cc_textures_path', nargs='?', default="cctextures", help="Path to downloaded cc textures")
parser.add_argument('output_dir', nargs='?', default="/home/ziyu/shuang/gdrnpp_bop2022/datasets/BOP_DATASETS", help="Path to where the final files will be saved ")
...
target_bop_objs = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'toy'), model_type='cad', mm2m = True)
...
bproc.loader.load_bop_intrinsics(bop_dataset_path = os.path.join(args.bop_parent_path, 'toy'))
...
for i in range(args.num_scenes):
    ...
    for obj in (sampled_target_bop_objs):    
        ...
        if name == 'obj_000002':
            blue_col = np.random.uniform(0, 0.1)   
            mat.set_principled_shader_value("Base Color", [blue_col, 2*blue_col, 6*blue_col, 1])    
        elif name ==  'obj_000001':
            gray_col = np.random.uniform(0.1, 0.2)   
            mat.set_principled_shader_value("Base Color", [gray_col, 2*gray_col, 2*gray_col, 1])    
        elif name == 'obj_000003' or name == 'obj_000004' or name == 'obj_000005' or name == 'obj_000006': 
            white_col = np.random.uniform(0.8, 1)   
            mat.set_principled_shader_value("Base Color", [white_col, white_col, white_col, 1])      
    ...
    def sample_initial_pose(obj: bproc.types.MeshObject):
        ...
        elif name == 'obj_000002':
            obj.set_rotation_euler([np.pi/2,0,0])
        elif name == 'obj_000003':
            obj.set_rotation_euler([np.pi/2,0,0])
        elif name == 'obj_000004':
            obj.set_rotation_euler([np.pi/2,0,0])
        elif name == 'obj_000005':
            obj.set_rotation_euler([np.pi/2,0,0])
        elif name == 'obj_000006':
            obj.set_rotation_euler([np.pi/2,0,0])
